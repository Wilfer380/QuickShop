<x-app-layout>
    ...
    <section class="vehicle-form-page">
        <div class="vehicle-form-header">
            <div>
                ...
                <p class="page-subtitle">Actualiza la información del inventario sin perder el registro.</p>
            </div>
        ...
        <div class="vehicle-form-layout">
            <section class="vehicle-form-card">
                <form class="vehicle-form" action="{{ route('vehiculos.update', $vehiculo) }}" method="POST" enctype="multipart/form-data">
                    @method('put')
            ...
            <aside class="vehicle-form-side">
                <article class="vehicle-preview-card">
                    <div class="vehicle-preview-badge">Registro actual</div>
                    @if ($vehiculo->imagen)
                        <img src="{{ route('vehiculos.imagen', ['vehiculo' => $vehiculo, 'v' => optional($vehiculo->updated_at)->timestamp]) }}" alt="Foto vehículo" class="vehicle-preview-image vehicle-preview-image--cover">
                    @else
    ...
    @push('styles')
        <style>
            ...
            .vehicle-preview-image{height:170px;border-radius:12px;background:rgba(15,23,42,.75);border:1px solid rgba(148,163,184,.16);display:flex;align-items:center;justify-content:center;color:#3B82F6;object-fit:cover;width:100%}
            .vehicle-preview-image--cover{object-fit:cover}
