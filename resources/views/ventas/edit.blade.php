<x-app-layout>
    ...
    <section class="sale-form-page">
        <div class="sale-form-header">
            <div>
                <h1 class="page-title">Editar venta #{{ $venta->id }}</h1>
                <p class="page-subtitle">Ajusta datos comerciales sin crear pagos nuevos ni alterar el historial de abonos.</p>
            </div>
            <a href="{{ route('ventas.show', $venta) }}" class="btn-new-sale btn-secondary">Volver</a>
        </div>
        ...
        <div class="sale-form-layout">
            ...
            <aside class="sale-side-card">
                ...
                <h2>{{ $venta->cliente->nombres }} {{ $venta->cliente->apellidos }}</h2>
                <p>El estado de pago se recalcula con los abonos existentes. Si cambiás el vehículo, el anterior vuelve a disponible y el nuevo queda vendido.</p>
            </aside>
        </div>
    </section>
